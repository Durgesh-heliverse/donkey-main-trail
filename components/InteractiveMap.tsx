...
import { MapPin, Tent, Bed, Utensils, Mountain, ChevronDown, ChevronUp, X, ChevronLeft, ChevronRight, Play } from "lucide-react";
...
const typeColorMap: { [key: string]: string } = {
  start: "bg-amber-800",
  accommodation: "bg-green-700",
  viewpoint: "bg-stone-700",
  end: "bg-red-900",
};

export default function InteractiveMap({ data }: InteractiveMapProps) {
  const { title, highlightedText, description, mapUrl, legend, trailPoints } = data;
  ...
  const handleImageClick = useCallback((images: Array<{
    image?: { asset: any };
    videoUrl?: string;
    isYoutube?: boolean;
  }>, startIndex: number = 0) => {
  ...
  return (
    <section id="interactive-map" className="py-20 bg-gradient-to-b from-white to-gray-50">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        {/* Header */}
        <div className="text-center mb-12">
          <h2 className="text-4xl md:text-5xl font-bold text-gray-900 mb-4">
            {title && <span className="text-grey-900">{title} </span>}
            {highlightedText && <span className="text-orange-500">{highlightedText}</span>}
          </h2>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-1">
            <div className="bg-white rounded-2xl shadow-lg p-6 sticky top-24">
              <h3 className="text-xl font-bold text-gray-900 mb-4">Trail Points</h3>
              {trailPoints && trailPoints.length > 0 ? (
                <div className="space-y-3">
                  {trailPoints.map((point) => {
                    const IconComponent = iconMap[point.icon || "mapPin"] || iconMap.mapPin;
                    ...
                    const difficultyColor = difficultyColors[point.difficulty] || "bg-gray-100 text-gray-800";
                          ...
                          <div className="flex items-start gap-3">
                            ...
                            <div className="flex-1 min-w-0">
                              <div className="flex items-center justify-between mb-1">
                                <span className="text-sm font-medium text-gray-500">{point.day}</span>
                              ...
                              <div className="flex items-center gap-2 text-xs">
                                <span className={`px-2 py-1 rounded capitalize ${difficultyColor}`}>
                                  {point.difficulty}
                                </span>
                              </div>
                            </div>
                            ...
                            {point.images && point.images.length > 0 && (
                              <div className="mb-4 ">
                                <div className="grid grid-cols-4 gap-2">
                                  {point.images.slice(0, 4).map((imageItem, imgIndex) => (
                                    <button
                                      key={imgIndex}
                                      onClick={(e) => {
                                        e.stopPropagation();
                ...
                <p className="text-gray-500 text-sm">No trail points configured</p>
              )}
            </div>
          </div>
        </div>
        ...
        {imageModalOpen && selectedImages.length > 0 && (
          ...
          >
            ...
            >
              <button
                onClick={handleCloseImageModal}
                className="absolute top-4 right-4 z-10 p-2 bg-white bg-opacity-90 hover:bg-opacity-100 rounded-full transition-colors"
              >
              ...
              <div className="relative flex-1 bg-black rounded-lg overflow-hidden mb-4">
                {selectedImages[currentImageIndex]?.videoUrl ? (
                  ...
                  <div className="relative w-full h-full">
                    <Image
                      src={urlFor(selectedImages[currentImageIndex].image).url()}
                ...
                {selectedImages.length > 1 && (
                  <>
                    <button
                      ...
                      className="absolute left-4 top-1/2 -translate-y-1/2 bg-white bg-opacity-90 hover:bg-opacity-100 p-3 rounded-full shadow-lg transition-all z-10"
                    >
                      <ChevronLeft className="h-6 w-6 text-black" />
                    </button>
                    <button
                      onClick={handleNextImage}
                      className="absolute right-4 top-1/2 -translate-y-1/2 bg-white bg-opacity-90 hover:bg-opacity-100 p-3 rounded-full shadow-lg transition-all z-10"
                    >
                    ...
                    >
                      {imageItem.videoUrl ? (
                        <div className="w-full h-full flex items-center justify-center bg-gray-900 relative">
                          ...
                          {imageItem.image?.asset && (
                            <Image
                              src={urlFor(imageItem.image).width(80).height(80).url()}
